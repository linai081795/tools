$(function () {
    var urlStart = getUrlPrefix() + window.location.host + getGap(), courseId = getQueryString('course_id'), currentItemId = '', historyViewTime, currentTimeLen = 0,
        ...
        vipType = '', isShowEnergyInfo = '', currentMinStudyTime = '', djsTime = 0, showTime = $('#showTime'), questionK = $('#questionK'), questionFlag = true, 
        ifAllComplate = false, complateFlag = true, ifSetNext = Cookies.get('ifSetNext'), videoDiv = $('#videoDiv'), nowstatus = $('.nowstatus'), isFinish = '', 
        totalTime = '', prevsec = 0, ifchangeurl = false, isGetJF = false, qtitle = $('.qtitle'), qanswerBox = $('.qanswerBox'), questionBox = $('.questionBox'), 
        ttFlag = false, perTime = 60, loadLength = 0, theUserId = '', courseList = [], currentIndex = 0, ifShowTime = false;
    function events() {
        setDivHeight();
        getPower();
        ckboxChange();
    ...
    function addTool(){
        $(".vjs-control-bar").append('<button class="vjs-control " id="danmu_send_opt">'+
            '<select style="-webkit-appearance: none;background-color: silver;border-radius: 0px;font-size:12px;padding:2px;" id="changeSelect">' +
            '<option value="opel">高清</option>' +
        ...
        $('#changeSelect').on('change',function(){
            myPlayer.pause();
            $('#changeMask').removeClass('dnc');
            window.setTimeout(function(){
                $('#changeMask').addClass('dnc');
                myPlayer.play();
            },800);
        });
    }
    ...
    function getPower() {
        ...
        if (res.code == 1000) {
            ...
            if(data._isOpenTotalTimeShow === 1){
                $('.nowtime').removeClass('dnc');
                ifShowTime = true;
            }
        }
    }
    ...
    function getVideoInfo(siid, obj) {
        ...
        var title = data.lessonName, playUrl = data.playUrl;
        ...
        if (flag) {
            ...
            prevsec = 0;
            if(ifShowTime){
                showTime.text(timeToFormat(totalTime));
            }
            if (!isEmpty(playUrl)) {
                if (obj) {
    ...
    function time_fun() {
        timer = setInterval(function () {
            sec++;
            if(ifShowTime){
                let nowtime = totalTime + sec;
                showTime.text(timeToFormat(nowtime));
            }
            if(djsTime > 0){
                djsTime--;
            }
